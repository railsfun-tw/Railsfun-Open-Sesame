...
安裝完 Ruby 之後您應可以存取 `ruby`、`irb`、`gem` 等命令。

### `irb` 使用簡介
`irb` 為 Ruby 的互動式 shell（殼層）環境，執行 `irb` 後您就可以在裏面一行一行執行 Ruby 語言的語句，`irb` 把它執行之後將輸出的訊息印出來（如果有）並印出回傳值，很適合拿來練習寫 Ruby 程式。
